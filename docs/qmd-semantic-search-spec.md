...
Repo: `compliance-rag` (censimento obblighi/principi QTSP, SQLite + FastAPI in `app/web_ui.py`, schema in `app/schema.sql`). Leggere `CLAUDE.md` del repo prima di iniziare.
...
2. **Registrazione collection qmd**
   ```
   qmd collection add app/qmd_export --name compliance-rag --mask "**/*.md"
   qmd embed
   ```
...
4. **Wiring in `web_ui.py`**
   Nuovo endpoint, es. `GET /api/ricerca-semantica?q=...`, che invoca `qmd query <q> -c compliance-rag --json -n <k>` via `subprocess`, parsa il JSON risultato (contiene path/docid/score), estrae `id` e tipo (`obbligo`/`principio`) dal nome file, e joina con il DB per restituire le righe complete (stesso formato di `_riga_obbligo`/`_riga_principio`). Gestire: `qmd` non installato/non raggiungibile, timeout, nessun risultato.
