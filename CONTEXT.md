...
Il secondo tipo di nodo del censimento (accanto a Obbligo): una disposizione normativa che **non impone un comportamento a un soggetto**. Comprende sia le disposizioni che stabiliscono un effetto giuridico, una presunzione legale o un divieto di discriminazione (es. art. 25 eIDAS, "a una firma elettronica non sono negati gli effetti giuridici e l'ammissibilità come prova in giudizio per il solo motivo della sua forma elettronica"), sia le disposizioni di cornice — oggetto/scopo, ambito di applicazione, definizioni, finalità generali — che non impongono un comportamento né dichiarano un effetto giuridico specifico, ma che comunque fanno parte del testo normativo e vanno censite (es. art. 1 eIDAS, "Oggetto"). Non ha soggetto obbligato: è per questo che non rientra nella definizione di Obbligo, non per un limite di estrazione.

Attributi:
- **Fonte** + **riferimento puntuale**, come per l'Obbligo
- **Testo del principio** (estratto letterale o parafrasi normalizzata)
- **Tipo di principio**: non discriminazione, equivalenza giuridica, valore probatorio, presunzione legale, scopo/ambito di applicazione, definitorio, altro
- **Oggetto giuridico** a cui si applica — multi-valore, dall'insieme: firma elettronica (semplice/avanzata/qualificata), sigillo elettronico (semplice/avanzato/qualificato), marca temporale elettronica qualificata, documento elettronico, servizio di recapito elettronico certificato, identificazione elettronica, altro. Questo attributo è ciò che permette a una domanda come "può un giudice non considerare un documento firmato elettronicamente?" di trovare l'art. 25 anche se il testo letterale non contiene "giudice" o "documento": la domanda si aggancia all'oggetto giuridico, non solo al testo. Per le disposizioni di cornice (scopo, ambito, definizioni) che non si legano a un oggetto giuridico specifico questo attributo può restare vuoto: non è un requisito di inclusione.
...
Perimetro: qualunque norma delle Fonti censite che non sia un Obbligo (non solo quelle relative a strumenti gestiti dai QTSP) — decisione presa perché l'utente deve poter rispondere a quesiti operativi reali (es. di natura probatoria/processuale) con richiami puntuali alle norme, anche quando la norma non riguarda direttamente un obbligo di un QTSP. Questo allarga la Destination del progetto oltre i soli "obblighi applicabili ai QTSP" (vedi map.md).

### Copertura completa (nessun articolo scartato per assenza di soggetto obbligato o effetto giuridico dichiarato)

Ogni articolo/comma di una Fonte importata nel censimento deve corrispondere a **esattamente un nodo** (Obbligo o Principio), non solo gli articoli che impongono un comportamento o dichiarano un effetto giuridico in senso stretto. Un articolo come l'art. 1 eIDAS ("Oggetto") o un articolo di definizioni non va escluso: va censito come Principio (tipo "scopo/ambito di applicazione" o "definitorio", vedi sopra). Questa regola vale per l'estrazione **futura**: non implica il refill retroattivo automatico degli articoli oggi mancanti nel DB esistente — quello è un lavoro di estrazione granulare a parte, già speced in `docs/superpowers/specs/2026-09-11-censimento-granulare-design.md` (meccanismo `verifica_copertura`/`INDICE_ARTICOLI`/`MAPPATURA` per garantire che ogni comma del testo ufficiale sia coperto da esattamente una riga), non ancora eseguito. Vedi ADR-0007 per la decisione e il rimando.
...
- **Attua / è attuato da** — un nodo (tipicamente una regola tecnica di rango inferiore, es. un articolo DPCM) che dà attuazione operativa a un nodo di rango superiore (es. un articolo CAD)
- **Recepisce / è recepito da** — un nodo che incorpora formalmente, in un contesto normativo diverso, il contenuto di un altro nodo (non limitato al recepimento di una direttiva UE in una norma nazionale: include anche, ad esempio, una norma nazionale generale il cui contenuto viene recepito/ripreso all'interno di una normativa di settore o tecnica). Distinta da **Attua**: Attua aggiunge dettaglio operativo/tecnico ("come" applicare una norma di rango superiore, che resta l'unica fonte del precetto); Recepisce trasferisce il contenuto stesso di un nodo in un altro strumento normativo, che diventa fonte autonoma dello stesso precetto nel proprio ambito
- **Richiama / è richiamato da** — rinvio o citazione esplicita di un nodo verso un altro, senza modificarlo né sostituirlo
- **Si applica a / gli si applica** — un nodo (tipicamente un Principio che definisce una distinzione o un ambito) la cui portata si estende a uno o più altri nodi tracciati
- **Modifica / è modificato da** — variazione parziale di un nodo preesistente che resta comunque rilevante (distinta da **Sostituisce**, che rappresenta invece la sostituzione integrale del nodo precedente: usare Modifica quando il nodo "da" affianca/integra il nodo "a" senza renderlo interamente superato, Sostituisce quando il nodo "a" non è più applicabile)
- **Deroga a / è derogato da** — un nodo che esclude l'applicazione di un altro nodo in casi specifici, senza alterarne il testo (a differenza di **Modifica**) e senza abrogarlo (a differenza di **Abroga**): il nodo generale resta pienamente vigente al di fuori dei casi derogati. Distinta anche da **È condizionato da**, che riguarda l'applicabilità di un nodo legata a un fatto tracciato nel censimento, non un'eccezione esplicita posta da un altro nodo normativo
- **Abroga / è abrogato da** — soppressione di un nodo senza un nodo sostitutivo diretto tracciato nel censimento
- **Definisce / è definito da** — un nodo che introduce la definizione di un concetto rappresentato da un altro nodo tracciato
- **Sanziona / è sanzionato da** — un nodo che stabilisce la conseguenza sanzionatoria applicabile in caso di inosservanza di un altro nodo

Ogni relazione porta due metadati di provenienza sull'arco stesso (non sui nodi): `evidence_type` (`textual` = relazione esplicita nel testo normativo, es. un rinvio letterale; `inferred` = dedotta da un'estrazione LLM non ancora validata da un umano; `human-curated` = relazione verificata/costruita da un umano) e `confidence` (score opzionale 0–1, `NULL` quando non c'è uno score reale da riportare). Stessa semantica di provenienza di `stato_validazione` sui nodi Obbligo/Principio, ma per-arco invece che per-nodo.
